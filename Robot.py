...
class WPManager:
    # Assumption: CPs is a list of [ [x coord, y coord]  ]
...
class RobotSimulation:
    ...
    def __init__(self, robot, t0=0.0, tf=10.0, dt=0.01, CPs=0, wpHandles=0, WPlist=0, obst=0):
    ...
    def plotXY(self, figNo=1, xmin=-2, xmax=12, ymin=-2, ymax=12):
        ...
        if(self.wpHandles):
            for p in self.wpHandles:
                graph.plot(p[0], p[1], 'ro', color='b',
                           markersize=3, label='marker only')
        ...
        graph.plot(self.obst[0], self.obst[1], 'ro', color='r',
                   markersize=7, label='marker only')
